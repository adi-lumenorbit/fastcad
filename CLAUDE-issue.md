...
```bash
gh -R {OWNER/REPO} issue create --title "Title" --body-file tmp/issue-body.txt
```
...
```bash
gh -R {OWNER/REPO} issue edit N --title "NN — Title"
```

Example: `gh issue edit 42 -R {OWNER/REPO} --title "42 — New widget"`
...
GitHub issue: {REPO_URL}/issues/N
...
```
[NN-slug.md]({REPO_URL}/blob/main/docs/plans/NN-slug.md)
...
```bash
gh -R {OWNER/REPO} issue edit N --body-file tmp/issue-body.txt
```
...
- **Item column** — link to the plan file:
  `[Title]({REPO_URL}/blob/main/docs/plans/NN-slug.md)`
